// Static style presets (Phase 8) — colour palette, font pairing, layout
// variant. Not stored in KV; these are fixed, versioned-in-code constants.
// Duplicated (not imported) into astro/src/lib/presets.ts and
// admin/src/lib/presets.ts — worker/, astro/, and admin/ are three
// separate npm projects with no shared package, the same reason
// MarketConfig itself already has three independent copies across this
// codebase. Keep all three in sync by hand if these ever change.
